...
void lineToAssembly(vector<string> params){
    if(params[0].compare("pop_param")==0){
        ...
        assembly.push_back("LOAD $sp "+rd+" "+to_string(argument_pos+1));//+1 um por causa de $ra empilhado no inicio da funcao
        if(in_mem){
        ...
        if(scope.compare("main")!=0 && pushing_args==true){
        ...
        if(params[0].compare("goto")==0){
            ...
            params_args.push_back("STORE $sp "+rs+" "+to_string(params_amount));
            params_amount++;
        }
            ...
            assembly.push_back("ADDI $sp $sp 1");
            assembly.push_back("PARAMs");
            for(int i=0;i<params_args.size(); i++){
                assembly.push_back(params_args[i]);
            }
            params_args.clear();
            params_amount = 0;
